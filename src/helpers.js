...
export let funcBuilder = R.curry(
  (mongoFn, input, connectionString, collectionName) => {
    return R.curryN(input.length, (...args) => {
      let inputValue = input.apply(null, args);
      return mongoFnRunner(
        mongoFn(inputValue),
        collectionName,
        connectionString
      );
...
export let funcBuilder2 = R.curry(
  (mongoFn, query, update, connectionString, collectionName) => {
    return R.curryN(query.length, (...args) => {
      let queryValue = query.apply(null, args);
      if (typeof update === "object") {
        return mongoFnRunner(
          mongoFn(queryValue, update),
          collectionName,
          connectionString
        );
      } else {
        return R.curryN(query.length, (...args) => {
          ...
          return mongoFnRunner(
            mongoFn(queryValue, updateValue),
            collectionName,
            connectionString
          );
        });
...
export function stats(connectionString) {
  return getConnection(connectionString).then(
    db =>
      new Promise((resolve, reject) =>
        db.stats(resultResolver(resolve, reject))
      )
  );
}
